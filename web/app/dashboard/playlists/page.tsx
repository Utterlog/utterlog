...
import { MusicNote, Plus, Edit2, Trash2, Search, X } from '@/components/icons';
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <MusicNote size={20} className="text-primary-themed" />
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          ...
          <Button onClick={openCreate}><Plus size={16} />创建歌单</Button>
        </div>
      </div>
        ...
        <div style={{ width: '300px', flexShrink: 0 }}>
          {loading ? (
            ...
            <div className="text-dim" style={{ textAlign: 'center', padding: '40px' }}>
              ...
              <Button onClick={openCreate} size="sm"><Plus size={14} />创建</Button>
            </div>
          ) : (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: '2px', flexShrink: 0 }}>
                      <button onClick={(e) => { e.stopPropagation(); openEdit(p); }} className="text-primary-themed" style={{ padding: '4px', background: 'none', border: 'none', cursor: 'pointer' }}><Edit2 size={13} /></button>
                      <button onClick={(e) => { e.stopPropagation(); setDeleteId(p.id); }} style={{ padding: '4px', background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626' }}><Trash2 size={13} /></button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {activePlaylist ? (
            <div className="card" style={{ padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                ...
                <Button size="sm" onClick={loadAllMusic}><Plus size={14} />添加歌曲</Button>
              </div>
                ...
                <div>
                  {playlistSongs.map((s: any, i: number) => (
                    <div key={s.id} style={{
                      ...
                      <button onClick={() => removeSong(s.id)} style={{ padding: '4px', background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', flexShrink: 0 }}><X size={14} /></button>
                    </div>
                  ))}
                </div>
              )}
            ...
            {filteredMusic.length === 0 ? (
              ...
              <div key={m.id} style={{
                ...
                {songIds.has(m.id) ? (
                  ...
                  <Button size="sm" onClick={() => addSongToPlaylist(m.id)}><Plus size={12} /></Button>
                )}
              </div>
            ))}
          </div>
