...
<div class="container list-page">
    <div class="page-header" style="margin-bottom: 24px;">
        ...
        <p class="page-desc" style="color: #666; font-weight: 600; font-family: monospace;"><?= (int)($tag['post_count'] ?? ($feed['total'] ?? ($pager['total'] ?? 0))) ?> 篇文章</p>
    </div>

    <?php if (empty($posts)): ?>
        <div class="empty-state" style="padding: 40px 0; font-weight: 800;"><p>该标签暂无文章</p></div>
    <?php else: ?>
        <section class="post-list" id="js-list-feed" data-list-feed
                 data-endpoint="<?= e($feed['endpoint'] ?? url('api/tag-posts')) ?>"
                 data-query-key="<?= e($feed['query_key'] ?? 'tag') ?>"
                 data-query-value="<?= e($feed['query_value'] ?? ($tag['slug'] ?? '')) ?>"
