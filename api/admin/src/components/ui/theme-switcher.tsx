...
export function ThemeSwitcher() {
  ...
  const handleSwitch = (t: typeof themes[0]) => {
    setTheme(t.key);
    setToast(`已切换为${t.label}主题`);
      ...
      {themes.map((t) => (
        <button
          ...
          style={{
            ...
            backgroundColor: t.color, border: 'none', cursor: 'pointer',
            transition: 'transform 0.15s',
            boxShadow: theme === t.key
              ? `0 0 0 2px var(--color-bg-card), 0 0 0 3.5px ${t.color}`
              : 'none',
          }}
        />
      ))}
