...
export default function PostsPage() {
  ...
  const columns: any[] = [
    ...
    ), width: '32px', render: (row: any) => (
      <input type="checkbox" checked={selected.has(row.id)} onChange={() => toggleSelect(row.id)} />
    )},
    { key: 'id', title: '#', width: '32px', render: (row: any) => (
    ...
    ), width: '34%', render: (row: any) => {
      const cat = row.categories?.[0];
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          {cat?.icon ? (
            <i className={cat.icon} title={cat.name} style={{ fontSize: '13px', color: 'var(--color-primary)', flexShrink: 0, width: '16px', textAlign: 'center' }} />
          ) : (
            <i className="fa-regular fa-file-lines" style={{ fontSize: '13px', color: 'var(--color-text-dim)', flexShrink: 0, width: '16px', textAlign: 'center' }} />
          )}
    ...
    { key: 'tags', title: '关键词', width: '50px', render: (row: any) => {
      const tags = row.tags || [];
      if (!tags.length) return <span className="text-dim" style={{ fontSize: '11px' }}>-</span>;
      return (
        <span title={tags.map((t: any) => t.name).join(', ')} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: 'var(--color-text-sub)', cursor: 'default' }}>
          <i className="fa-regular fa-tags" style={{ fontSize: '11px' }} />{tags.length}
        </span>
      );
    }},
    { key: 'created_at', title: '时间', width: '80px', render: (row: any) => <span className="text-dim" style={{ fontSize: '12px' }}>{formatDate(row.created_at)}</span> },
    ...
    { key: 'actions', title: <span style={{ textAlign: 'right', display: 'block' }}>操作</span>, width: '150px', render: (row: any) => (
      <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
        <button onClick={() => navigate(`/posts/edit/${row.id}`)} className="action-btn" title="编辑"><i className="fa-regular fa-pen" style={{ fontSize: '14px' }} /></button>
        ...
        <button
          ...
          className={`action-btn${row.status === 'draft' ? ' active' : ''}`}
          title={row.status === 'draft' ? '取消草稿（发布）' : '移至草稿箱'}
          style={row.status === 'draft' ? { color: 'var(--color-primary)' } : undefined}
        ...
        <button
          ...
          className={`action-btn${row.status === 'private' ? ' active' : ''}`}
          title={row.status === 'private' ? '取消私密（发布）' : '设为私密'}
          style={row.status === 'private' ? { color: '#d97706' } : undefined}
